...
public class UserController {
    ...
    AuthResponse loginUser(@RequestBody AuthRequest authRequest) {
        ...
        if(user.get(0).getPassword().equals(authRequest.getPassword())) {
            long id = 0;
            switch (user.get(0).getRole()) {
                case CLIENT:
                    id = user.get(0).getClient().getClient_id();
                    break;
                case MANAGER:
                    id = user.get(0).getManager().getManager_id();
                    break;
                case REPAIRER:
                    id = user.get(0).getRepairer().getRepairer_id();
                    break;
                case DEALER:
                    id = user.get(0).getDealer().getDealer_id();
                    break;
            }
